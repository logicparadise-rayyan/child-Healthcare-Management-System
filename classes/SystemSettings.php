...
class SystemSettings extends DBConnection{
	...
	function set_EnrollerData($field='',$value=''){
		if(!empty($field) && !empty($value)){
			$_SESSION['EnrollerData'][$field]= $value;
		}
	}
	function EnrollerData($field = ''){
		if(!empty($field)){
			if(isset($_SESSION['EnrollerData'][$field]))
				return $_SESSION['EnrollerData'][$field];
			else
				return null;
		}else{
			return false;
	...
	function Enroller_sess_des(){
		if(isset($_SESSION['EnrollerData'])){
				unset($_SESSION['EnrollerData']);
			return true;
		}
			return true;
	}
